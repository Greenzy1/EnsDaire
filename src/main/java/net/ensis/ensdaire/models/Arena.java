...
import net.ensis.ensdaire.utils.FX;
import net.ensis.ensdaire.utils.SB;
import net.ensis.ensdaire.utils.VersionUtils;
import org.bukkit.*;
...
public class Arena {
    ...
    private final Map<CircleColor, Location> capsuleMap = new HashMap<>(); 
    ...
    public Location getCapsuleSpawn(CircleColor color) {
        return capsuleMap.get(color);
    }

    public void setCapsuleSpawn(CircleColor color, Location loc) {
        capsuleMap.put(color, loc);
    }
    
    public Collection<Location> getAllCapsuleSpawns() { // İSİM DEĞİŞTİRİLDİ
        return capsuleMap.values();
    }
    ...
    public JoinResult addPlayer(Player player) {
        ...
        broadcast(plugin.getLanguageManager().getMessage("join")
        ...
        net.ensis.ensdaire.gui.TeamSelectGui.open(plugin, player, this);
        checkAutoStart();
    ...
    private JoinResult joinAsSpectator(Player player) {
        ...
        bossBar.addPlayer(player);
        player.sendMessage(plugin.getLanguageManager().getMessage("spectator"));
    ...
    public void removePlayer(Player player) {
        ...
        broadcast(plugin.getLanguageManager().getMessage("leave"));
    ...
    private void cancelCountdown() {
        ...
        bossBar.hide();
        broadcast(plugin.getLanguageManager().getMessage("not-enough-players").replace("{min}", String.valueOf(minPlayers)));
        SB.updateLobbyAll(plugin, this);
    }
    ...
    private void teleportToCapsules() {
        for (GamePlayer gp : players.values()) {
            ...
            Location loc = capsuleMap.get(gp.getColor());
            if (loc == null) loc = lobbySpawn; 
            
            gp.setCapsuleLocation(loc);
            p.teleport(loc);
            ...
            circleManager.createCircle(gp.getUuid(), loc, gp.getColor());
            applySpawnEffects(p);
    ...
    private void nextRound() {
        ...
        broadcast(plugin.getLanguageManager().getMessage("modifier-start").replace("{name}", currentModifier.getDisplayName()));
        broadcast(plugin.getLanguageManager().getMessage(currentModifier.getDescriptionKey()));

        spawnCenterShulkers();
        broadcast(plugin.getLanguageManager().getMessage("round-start").replace("{round}", String.valueOf(currentRound)));
    ...
    private void startCircleCheck() {
        ...
        int interval = plugin.getConfig().getInt("game.circle-check-interval", 2); 
        circleTask = new BukkitRunnable() {
            @Override public void run() {
                ...
                for (GamePlayer gp : new ArrayList<>(players.values())) {
                    ...
                    Player p = Bukkit.getPlayer(gp.getUuid());
                    if (p == null) continue;
                    if (!circleManager.isSafe(gp.getUuid(), p.getLocation(), gp.getColor())) {
                        eliminate(p, null, EliminationCause.CIRCLE);
                    }
                }
            }
    ...
    private void spawnCenterShulkers() {
        if (lobbySpawn == null) return;
        int heightOffset = plugin.getConfig().getInt("shulker.spawn-height-offset", 0);
        Location center = lobbySpawn.clone().add(0, heightOffset, 0); 
        List<Location> grid = new ArrayList<>();
        for (int x = -1; x <= 1; x++) {
            for (int z = -1; z <= 1; z++) {
                grid.add(center.clone().add(x, 0, z));
            }
        }
        shulkerManager.spawnAll(grid);
    }
    ...
    private void endRound() {
        ...
        for (GamePlayer gp : getAlivePlayers()) {
            ...
            Player p = Bukkit.getPlayer(gp.getUuid());
            if (p != null) p.sendMessage(plugin.getLanguageManager().getMessage("token-earned")
                    .replace("{amount}", String.valueOf(surviveTokens))
                    .replace("{total}", String.valueOf(plugin.getPlayerDataManager().getTokens(gp.getUuid()))));
        }
        
        broadcast(plugin.getLanguageManager().getMessage("round-end").replace("{round}", String.valueOf(currentRound)));
    ...
    public void eliminate(Player victim, Player killer, EliminationCause cause) {
        ...
        switch (cause) {
            case CIRCLE -> broadcast(plugin.getLanguageManager().getMessage("eliminated-circle") + " §8(§7" + gp.getColoredName() + "§8)");
            case KILLED -> {
                broadcast(plugin.getLanguageManager().getMessage("eliminated-killed").replace("{killer}", killer != null ? killer.getName() : "?") + " §8(§7" + gp.getColoredName() + "§8)");
                if (killer != null) {
                    handleKill(killer, gp);
                    plugin.getCosmeticManager().playKillEffect(victim, killer);
                }
            }
            case DISCONNECT -> broadcast("§7" + gp.getColoredName() + " §7bağlantısı kesildi.");
        }
        
        victim.sendMessage(cause == EliminationCause.CIRCLE ? plugin.getLanguageManager().getMessage("eliminated-circle") : plugin.getLanguageManager().getMessage("eliminated-killed").replace("{killer}", killer != null ? killer.getName() : "?"));
    ...
    private void handleKill(Player killer, GamePlayer victim) {
        ...
        if (!firstBloodDone) {
            ...
            killTokens += fbBonus;
            killPoints += fbPoints;
            broadcast(plugin.getLanguageManager().getMessage("first-blood").replace("{player}", killerGP.getColoredName()));
        }
        ...
        killer.sendMessage(plugin.getLanguageManager().getMessage("token-earned")
                .replace("{amount}", String.valueOf(killTokens))
                .replace("{total}", String.valueOf(plugin.getPlayerDataManager().getTokens(killer.getUniqueId()))));
    }
    ...
    private void finishGame() {
        ...
        if (winner != null) {
            ...
            broadcast(plugin.getLanguageManager().getMessage("win").replace("{player}", winner.getColoredName()));
    ...
    private void showResults() {
        List<GamePlayer> sorted = players.values().stream()
                .filter(gp -> gp.getPlacement() > 0)
                .sorted(Comparator.comparingInt(GamePlayer::getPlacement))
    ...
    public List<Location> getCapsuleLocations() { return capsuleMap.values().stream().toList(); } // İSİM DEĞİŞTİRİLDİ
